...
import { stockLevelEnum } from '@services/database/schemas';

import type { TypedContext } from '@repo/server-utils/types/app.types';

const countedBoxesSchema = z.coerce
  .number()
...
const monthEndCountItemSchema = z.object({
  ...
  countedBoxes: countedBoxesSchema,
});
...
const monthEndCountSortSchema = createSortSchema([
  'warehouseName',
  'brochureName',
  'brochureTypeName',
  'customerName',
...
export const listMonthEndCountsValidator = createValidatorSchema({
  query: paginationSchema
    ...
    .extend(monthEndCountSortSchema.shape)
    .extend({
      month: z.coerce
        .number()
      ...
      stockLevel: z.enum(stockLevelEnum.enumValues).optional(),
    }),
});
